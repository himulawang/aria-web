    ...
    <div class="flex h-screen overflow-hidden bg-base-200">
      ...
      >
        <div class="flex items-center justify-between mb-4">
          <div
            class={`px-2 py-2 text-xl font-bold ${shouldCollapse() ? "hidden" : "block"}`}
          >
            AriaWeb
          </div>
          <button
            class={`btn btn-ghost btn-xs ${isMobile() ? "hidden" : "block"}`}
              ...
              item.view === "settings" ? (
                <li class="relative">
                  <button
                    ...
                    class={`w-full flex items-center ${!shouldCollapse() && props.currentView === "settings" ? "active" : ""}`}
                  >
                    <item.icon class="w-5 h-5" />
                    <span class={shouldCollapse() ? "hidden" : "block"}>
                      {item.label()}
                    </span>
                  </button>
                  {/* Popover */}
                  <Portal>
                    <Show when={shouldCollapse() && isPopoverOpen()}>
