...
const DEFAULT_CONFIG: Aria2Config = {
    url: "http://192.168.68.58:6800/jsonrpc",
    token: "123",
    useWebSocket: false,
    httpMethod: "POST",
...
const [state, setState] = createStore<Aria2State>({
    config: StorageService.get<Aria2Config>("aria2_config") || DEFAULT_CONFIG,
    ...
    globalStat: null,
    appSettings:
        StorageService.get<Record<string, any>>("ariang_settings") || {},
});
...
export const aria2Store = {
    ...
    async updateConfig(newConfig: Partial<Aria2Config>) {
        ...
        StorageService.set("aria2_config", updated);

        this.stopPolling();
        client = new Aria2Client(updated);
    },
    async connect() {
        if (!client) {
            client = new Aria2Client(state.config);
        }
    ...
    async init() {
        logger.info("Initializing and attempting auto-connect...", LOG_CONTEXT);
        try {
            await this.connect();
        } catch (e) {
            logger.warn(
    ...
    async fetchActiveTasks() {
        ...
        try {
            ...
            setState("tasks", (prev) => {
                ...
                return [
                    ...active,
                    ...prev.filter((t) => !activeGids.has(t.gid)),
                ];
            });
        } catch (e) {
    ...
    async fetchGlobalStat() {
        if (!client) await this.connect();
        try {
            const stat = await client!.request<GlobalStat>(
                "aria2.getGlobalStat",
            );
    ...
    async fetchTaskDetail(gid: string) {
        if (!client) await this.connect();
        try {
            const detail = await client!.request<any>("aria2.tellStatus", [
                gid,
            ]);
            setState("selectedTaskDetail", detail);
        } catch (e) {
            logger.error(
                `Failed to fetch detail for task ${gid}: ${e}`,
                LOG_CONTEXT,
            );
            throw e;
        }
    ...
    async getGlobalOption(name: string) {
        ...
        try {
            ...
            logger.error(
                `Failed to get global option ${name}: ${e}`,
                LOG_CONTEXT,
            );
            throw e;
        }
    ...
    async changeGlobalOption(name: string, value: any) {
        if (!client) await this.connect();
        try {
            await client!.request("aria2.changeGlobalOption", [name, value]);
        } catch (e) {
            logger.error(
                `Failed to change global option ${name}: ${e}`,
                LOG_CONTEXT,
            );
            throw e;
        }
    ...
    async limitDownloadSpeed(gid: string | null, limit: number) {
        ...
        try {
            const method = gid
                ? "aria2.limitDownloadSpeed"
                : "aria2.changeGlobalOption";
            const params = gid ? [gid, limit] : ["download-limit", limit];
    ...
    async limitUploadSpeed(gid: string | null, limit: number) {
        ...
        try {
            const method = gid
                ? "aria2.limitUploadSpeed"
                : "aria2.changeGlobalOption";
            const params = gid ? [gid, limit] : ["upload-limit", limit];
            await client!.request(method, params);
        } catch (e) {
    ...
    setupEventListeners() {
        ...
        client.on("aria2.onDownloadComplete", (params: any[]) => {
            ...
            this.fetchTaskDetail(gid).catch((err) =>
                logger.error(
                    `Error in onDownloadComplete: ${err}`,
                    LOG_CONTEXT,
                ),
            );
